...
function amd(loader) {
    ...
    var amdRegEx2 = /(?:^\uFEFF?|[^$_a-zA-Z\xA0-\uFFFF.])(\\"|"(?:\\"|[^"])*")|define\s*\(\s*("[^"]+"\s*,\s*|'[^']+'\s*,\s*)?\s*(\[(\s*(("[^"]+"|'[^']+')\s*,|\/\/.*\r?\n|\/\*(.|\s)*?\*\/))*(\s*("[^"]+"|'[^']+')\s*,?)?(\s*(\/\/.*\r?\n|\/\*(.|\s)*?\*\/))*\s*\]|function\s*|{|[_$a-zA-Z\xA0-\uFFFF][_$a-zA-Z0-9\xA0-\uFFFF]*\))/g;
    ...
    function getDefs(source, cb) {
        amdRegEx2.lastIndex = 0;
        var res,
            define,
            count = 0;

        while(res = amdRegEx2.exec(source)) {
            // 1 is always the name
            if(res[1])
                continue;

            var name = trimStr( res[2] ),
            ...
            if((requireIndex = deps.indexOf("require")) >= 0 ) {
                var factoryText = getFactoryText(source, res, amdRegEx2.lastIndex);
